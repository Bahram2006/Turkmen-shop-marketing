...
export const ProductCard = ({ name, price, image, category }: ProductCardProps) => {
  ...
  const handleLike = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    alert(`${name} halanlaryňyza goşuldy! ❤️`);
  };
    ...
    >
      <div className="relative aspect-[1/1.1] overflow-hidden rounded-[32px] bg-gray-50 mb-8">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-500">
          <button 
            onClick={handleLike}
            className="p-3 bg-white/90 backdrop-blur-md rounded-full shadow-sm hover:bg-red-500 hover:text-white transition-colors"
          >
            <Heart size={18} />
          </button>
        </div>
      ...
      <div className="space-y-3 px-2">
        <div className="flex justify-between items-start">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[2px] text-blue-600 mb-1">{category || "Täze Gelen"}</p>
            <h3 className="text-xl font-bold text-gray-900 tracking-tight leading-none group-hover:text-blue-600 transition-colors">{name}</h3>
          </div>
          <div className="text-right">
            <p className="text-2xl font-black text-gray-900 tracking-tighter">${Number(price).toFixed(2)}</p>
          </div>
        </div>
      </div>
    </motion.div>
